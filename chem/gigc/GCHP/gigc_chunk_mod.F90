...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!   
!   Codename Nuclear: GIGC_Chunk adaptor for GIGC <-> External Models.
!
!   This "GCHP" interface to restore original GIGC functionality is based on
!   studies of the original GEOS-Chem High Performance Code v11-02c code.
!
!   Major Updates:
!       4/25/2018     H.P. Lin        Re-engineer GIGC_Chunk_Mod to serve as a stateless entry point to GC
!          I have changed GIGC_CHUNK_MOD to be stateful about Input_Opt% and generic state info, which
!          will subsequently facilitate at an executive level calls to GIGC_Chunk_Run without the external
!          model needing to maintain a copy of StateChm, StateMet, StateDiag, HcoConfig, InputOpt as they
!          will be fed in and updated from external model data. This "statelessness" about dt-dependent data
!          enables more versatile use of GIGC as a column code.
!
!       6/3/2018      H.P. Lin        While GCHP is re-engineered to support stateless operation, we need to
!          write a few workarounds so that nested model runs in WRF are performed correctly in GIGC_Chunk_Init
!          and GIGC_Chunk_Mod. This involves updating CMN_SIZE_MOD's global variables upon entry to reflect the
!          appropriate sizes (I/J/LPAR, GLOB, etc.), replicating INIT_CMN_SIZE functionality. This introduces
!          code duplication - once we sync to (GI)GC upstream, this should be removed.
!
!       6/12/2018     H.P. Lin        GIGC_Chunk_Mod and other GEOS-Chem modules are re-engineered to remove any
!          in-module containers that potentially store state, and use GIGC_STATEFUL_MOD written by me to store them
!          in an unified container instead.
!
!       8/3/2018      H.P. Lin        Add all the recent developments in GCHP 12.0-dev (source syncing is a huge /joy/)
!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!
!   Original Authors: Harvard University Atmospheric Chemistry Modeling Group
!   Module(s): gigc_chunk_mod
!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
!
    ...
    ! them from the respective PET using GIGC_Stateful_Mod, which retrieves the objects
    ...
    ! as we need to replicate "input_mod"'s remaining functionality in GIGC_Stateful_Mod,
...
!
    ...
    ! This is performed using GIGC_Stateful_Mod now (hplin, 6/12/18)
    !-----------------------------------------------------------------------
   ...
   ! Initialize other GEOS-Chem modules
    ...
    !=======================================================================
    ! Save stateful information into GIGC_Stateful_Mod (hplin, 6/12/18)
    !=======================================================================
...
!
    ...
    USE Part_Mercury_Mod,   ONLY : PartitionHg  !yxzhang, 6/26/2019
...
!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    IF ( DoChem ) THEN
       ...
       ! Partition Hg(II) between gas and aerosol, yxzhang, 6/26/2019
       CALL PARTITIONHG( am_I_Root, Input_Opt, State_Chm, State_Met, RC )
