...
! 12/24/2020, xtxu, mercury chemistry needs HEMCO input
...
!
    ...
    ! 12/24/2020, xtxu, mercury needs
    REAL(f4), POINTER :: GC_BR(:,:,:)         =>NULL()
    REAL(f4), POINTER :: GC_BRO(:,:,:)        =>NULL()
    REAL(f4), POINTER :: GC_OH(:,:,:)         =>NULL()
    REAL(f4), POINTER :: GC_HO2(:,:,:)        =>NULL()
    REAL(f4), POINTER :: GC_CL(:,:,:)         =>NULL()     
    REAL(f4), POINTER :: GC_CLO(:,:,:)        =>NULL()      
    REAL(f4), POINTER :: GC_NO2(:,:,:)        =>NULL()      
    REAL(f4), POINTER :: GC_HOCL(:,:,:)       =>NULL()       
    REAL(f4), POINTER :: GC_O3(:,:,:)         =>NULL()     
    REAL(f4), POINTER :: GC_OA(:,:,:)         =>NULL()     
    REAL(f4), POINTER :: GC_JNO2(:,:,:)       =>NULL()       
    ...
    IF (Input_Opt%ITS_A_FULLCHEM_SIM) THEN   !  12/24/2020, xtxu, mercury need, connecting HEMCO

      CALL HCO_GetPtr( am_I_Root, HcoState, 'BR_GC',   GC_BR,RC)     ! 1/21/2021, xtxu       
      CALL HCO_GetPtr( am_I_Root, HcoState, 'BRO_GC',  GC_BRO,RC)     
      CALL HCO_GetPtr( am_I_Root, HcoState, 'OH_GC',   GC_OH,RC)     
      CALL HCO_GetPtr( am_I_Root, HcoState, 'HO2_GC',  GC_HO2,RC)     

      CALL HCO_GetPtr( am_I_Root, HcoState, 'CL_GC',   GC_CL,RC)
      CALL HCO_GetPtr( am_I_Root, HcoState, 'CLO_GC',  GC_CLO,RC)
      CALL HCO_GetPtr( am_I_Root, HcoState, 'NO2_GC',  GC_NO2,RC)
      CALL HCO_GetPtr( am_I_Root, HcoState, 'HOCL_GC', GC_HOCL,RC)
      CALL HCO_GetPtr( am_I_Root, HcoState, 'O3_GC',   GC_O3,RC)
      CALL HCO_GetPtr( am_I_Root, HcoState, 'OA_GC',   GC_OA,RC)
      CALL HCO_GetPtr( am_I_Root, HcoState, 'JNO2_GC', GC_JNO2,RC)
    ENDIF
...
100 format('No. of function calls:', i6, /,                                 &
    ...
    DO I = 1, IIPAR
       ...
       IF ( Input_Opt%ITS_A_FULLCHEM_SIM ) THEN !  12/24/2020, xtxu, mercury needs

         CL_off   = GC_CL(I,J,L) * 1e-9_fp * NUMDEN            !ppbv     ! 1/20/2021, xtxu
         CLO_off  = GC_CLO(I,J,L) * 1e-9_fp * NUMDEN           !ppbv  
         NO2_off  = GC_NO2(I,J,L) * 1e-9_fp * NUMDEN           !ppbv 
         HOCL_off = GC_HOCL(I,J,L) * 1e-9_fp * NUMDEN          !ppbv
         O3_off   = GC_O3(I,J,L) * NUMDEN                      !v/v   
         OA_off   = GC_OA(I,J,L)   
         JNO2_off = GC_JNO2(I,J,L) 

         IF (L .GE. 30) THEN
            BR_off   = GC_BR(I,J,L) * 1e-12_fp * NUMDEN * 1e5     !pptv 
            BRO_off  = GC_BRO(I,J,L) * 1e-12_fp * NUMDEN          !pptv
            OH_off   = GC_OH(I,J,L) * 1e-6_fp * ( AVO / 0.017)    !kg/m3
            HO2_off  = GC_HO2(I,J,L) * NUMDEN                     !v/v 

         ELSE IF (L .GT. 23 .AND. L .LT. 30) THEN
            BR_off   = GC_BR(I,J,L) * 1e-9_fp * NUMDEN * 0.001          !ppbv
            BRO_off  = GC_BRO(I,J,L) * 1e-9_fp * NUMDEN           !ppbv 
            OH_off   = GC_OH(I,J,L) * 1e-6_fp * ( AVO / 0.017)    !kg/m3
            HO2_off  = GC_HO2(I,J,L) * NUMDEN                     !v/v 
         ELSE 
            BR_off   = GC_BR(I,J,L) * 1e-9_fp * NUMDEN            !ppbv
            BRO_off  = GC_BRO(I,J,L) * 1e-9_fp * NUMDEN            !ppbv 
            OH_off   = GC_OH(I,J,L) * 1e-6_fp * ( AVO / 0.017)    !kg/m3
            HO2_off  = GC_HO2(I,J,L) * NUMDEN                     !v/v 
         ENDIF
...
!      IF ( I .EQ. 1 .AND. J .EQ. 1 .AND. L .EQ. 14) THEN
!          WRITE(*,*) 'Br Concentration: ',L, BR_off
!          WRITE(*,*) 'BrO Concentration: ', BRO_off 
!          WRITE(*,*) 'OH Concentration: ', OH_off  
!          WRITE(*,*) 'Cl Concentration: ', CL_off  
!          WRITE(*,*) 'ClO Concentration: ', CLO_off 
!          WRITE(*,*) 'HO2 Concentration: ', HO2_off 
!          WRITE(*,*) 'NO2 Concentration: ', NO2_off 
!          WRITE(*,*) 'HOCl Concentration: ', HOCL_off
!          WRITE(*,*) 'O3 Concentration: ', O3_off  
!          WRITE(*,*) 'OA Concentration: ', OA_off  
!          WRITE(*,*) 'JBrO Concentration: ', JBRO_off
!          WRITE(*,*) 'JNO2 Concentration: ', JNO2_off
!      ENDIF
