...
!
      ...
      ! Mercury chemistry, yxzhang, 6/22/2019
      Real(fp)           :: K_AQ_OX(5)
	 ...
	 ! NOTE: this extension of calculation in troposphere has been removed
      ...
      ENDIF
         ...
         ! Mercury chemistry, yxzhang, 6/22/2019
         !----------------------------------------------------------------
         K_AQ_OX = GET_HGAQ_RATE( I, J, L, State_Met )
         HET(ind_Hg0,1) = K_AQ_OX(1)
         HET(ind_Hg0,2) = K_AQ_OX(2)
         HET(ind_Hg0,3) = K_AQ_OX(3)
         HET(ind_Hg2,1) = K_AQ_OX(4)
         HET(ind_HgP,1) = K_AQ_OX(5)
...
!\\                                                                            
! !INTERFACE:                                                                  
      FUNCTION GET_HGAQ_RATE(I, J, L, State_Met) &
               RESULT( K_AQ_OX )
!                                                                              
! !USES:                                                                       
!                                                                              
      ...
      USE AEROSOL_MOD,        ONLY : OCPI, OCPO, SOAS
!                                                                              
! !INPUT PARAMETERS:                                                           
!                                                                                                       
...
! !LOCAL VARIABLES:                                                            
      ...
      REAL(fp)               :: KH_Hg0, KH_Hg2, KH_HOCl, KH_O3, delta_OH
      REAL(fp)               :: FC,     LWC,     C_OA
      REAL(fp)               :: F_Hg0,  F_HOCl,  F_O3,  F_Hg2
      REAL(fp)               :: R_HOCl, R_OH,    R_O3,  R_Hg2
      REAL(fp)               :: Nair, T, P, RH
!                                                                              
! !DEFINED PARAMETERS:                                                         
!                                                                              
      ...
      ! Hg(0):                                                              
      KH_Hg0 = 1.28e-1_fp * exp(2482e0_fp * ((1e0_fp/T) - 1e0_fp/298.15e0_fp))
      ! Toyota et al. 2014 citing Sanemasa 1975                             
      ! or KH_Hg0 = 1.1E-1 ! Hynes et al. 2009  

      ! Hg(2):
      KH_Hg2 = 1.4e4_fp * exp(5300.0e0_fp * (1e0_fp/T - 1e0_fp/298.15e0_fp))

      ! HOCl:                                                               
      KH_HOCl = 6.6e2_fp * exp(5900e0_fp * ((1e0_fp/T)-1e0_fp/298.15e0_fp))
      ...
      ! O3:                                                                 
      KH_O3 = 1.1e-2_fp * exp(2400e0_fp * ((1e0_fp/T) - 1e0_fp/298.15e0_fp))
      ! Jacob, 1986                                                          
      ...
      F_Hg2 = 1e0_fp/(1e0_fp+KH_Hg2*R_L_atm*T*LWC) ! unitless              
      ...
      ! Two-body reaction constant, yxzhang, 6/22/2019
      R_HOCl = kHOCl * P/1013e0_fp * 1e0_fp/Nair * F_Hg0 * KH_Hg0 * P/1013e0_fp / Nair * F_HOCl * KH_HOCl
      ...
      !&                 * delta_OH * OH
      !Two-body reaction constant, yxzhang, 6/22/2019   
      R_OH = kOH * P/1013e0_fp * 1e0_fp/Nair * F_Hg0 * KH_Hg0 * delta_OH
      ...
      !&            * O3 / Nair * F_O3 * KH_O3
      !Two-body reaction constant, yxzhang, 6/22/2019    
      R_O3 = kO3 *P/1013e0_fp *1e0_fp/Nair *F_Hg0 *KH_Hg0*P/1013e0_fp / Nair * F_O3 * KH_O3
      ...
      !------------------------!
      !(1) Hg(2)(aq) + hv -> Hg(0)
      ! C_OA unit is ug/m3, need to convert from kg/m3
      ! Simple SOA scheme for Standard chemistry
      ! Horowitz use diag42 tracer 6
      ! Not STP
      C_OA = (OCPO(I,J,L) + OCPI(I,J,L) + SOAS(I,J,L)) * 1.0e+9_fp
      R_Hg2 = ZPJ(L,11,I,J)*C_OA
      !write(6,*)'ZPJ:', L, I, J, ZPJ(L,11,I,J)
      !open(unit=6,file='zpj.txt')
      !write(6,*)'ZPJ:', L, I, J, ZPJ(L,11,I,J) 
      ! calculate first-order effective rate constant w.r.t. Hg2 for volume of air
      K_AQ_OX(4) = R_Hg2 * (1.0_fp-F_Hg2) * FC
      !------------------------!                                             
      ! HgP reduction          !                                            
      !------------------------!
      !(1) Hg(P) + hv -> Hg(0)
      ! calculate first-order effective rate constant w.r.t. HgP for volume of air
      IF ( RH .GT. 35.0e0_fp ) THEN
          K_AQ_OX(5) = R_Hg2
      !    write(6,*)'K_AQ_OX:', K_AQ_OX(5)        
      ELSE
          K_AQ_OX(5) = 0.0e0_fp 
      ENDIF
